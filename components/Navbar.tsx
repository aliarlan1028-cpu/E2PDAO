    ...
    <header className={`v4-nav-shell${open ? " is-open" : ""}`}>
      <div className="v4-wrap v4-nav">
        ...
        <div className="v4-nav-actions">
          <a href={E2P_LINKS.project("navbar_v5")} className="v4-nav-cta"><span>Start a project</span> <ArrowUpRight size={13} /></a>
      ...
      {open && <nav className="v4-mobile-nav" aria-label="Mobile navigation"><Link href="/">Home</Link>{NAV_LINKS.map((link) => <Link key={link.name} href={link.href}>{link.name}</Link>)}<a href={E2P_LINKS.project("navbar_mobile_v5")}>Start a project <ArrowUpRight size={14} /></a></nav>}
    </header>
  );
}
